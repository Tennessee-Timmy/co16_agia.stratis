...
if ((getpos leader _group distance _oldWP < 5 || _oldWP distance [0, 0, 0] < 10) && alive leader _group) then {
	...
	if (_transType isEqualTo 1) exitWith {
		...
		_wp setWaypointStatements ["true", "(group this) spawn {sleep 10; deleteWaypoint ((waypoints _this) select 1);_this setVariable ['aiMaster_cacheTrans_delete',true];_this setVariable ['aiMaster_transDone',true];};"];
