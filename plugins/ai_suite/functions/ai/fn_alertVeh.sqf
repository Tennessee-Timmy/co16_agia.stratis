...
if ((_enemySpotted distance _vehicle) < 150) then {
...
_vehGroups append aiMaster_fightingGroupsVeh;
{
	...
	if ((alive leader _xGroup) && ((leader _xGroup) distance _leader < 1500) && (side _leader isEqualTo side leader _xGroup)) then {
		...
		if ((_xLeader distance _leader < 400) && (side _xLeader isEqualTo side _leader)) then {
			_xBeh = "AWARE";
			_xSpeed = "NORMAL";
...
} count _infGroups;
{
	...
	if ( (alive leader _xGroup) && (leader _xGroup distance _leader < 2000) && (side _leader isEqualTo side leader _xGroup)) then {
		...
		private _xBeh = _xStance;
		if ((_xLeader distance _leader < 500) && (side _xLeader isEqualTo side _leader)) then {
			_xBeh = "AWARE";
